/**
 * The projection every user-visible state is rendered from.
 *
 * Each state carries its label and attention from the spec's tables, and each
 * degraded state carries the four sentences AC-0089 to AC-0092 require: what
 * was looked for, what was found instead, whose gap it is, and whether retrying
 * can change the answer.
 *
 * `inspection-stopped` is the one condition whose attribution and retryability
 * are **per reason** rather than per state. The table says so in both columns,
 * and projecting one answer for all of its reasons would attribute a network
 * timeout to the repository — which is the crossing AC-0093 forbids.
 */
