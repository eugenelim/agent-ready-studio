...
| --- | --- | --- | --- | --- |
| AC-0001 | met | S | source-identity.ts:94-117; source-inspection.ts:314-332; source-inspection.test.ts:71-74 | connect derives owner/repository from the submitted URL and records them; passing empty strings on the accepted branch reddens |
...
| AC-0008 | met | S | git-driver.ts:143-147; git-driver.test.ts:53-71; source-inspection.ts:244-248 | resolveRevision on the live pipeline re-applies the ref charset to the remote-reported ref over seven hostile names |
...
| AC-0010 | **not met** | W | source-inspection.ts:273-279; source-inspection.test.ts:89; runtime-child.ts:1117 | resolution leg strongly bound, but the materialization leg asserts toContain against a URL that already satisfies it; fetchUrl: url stays green |
| AC-0011 | met | S | git-driver.ts:83,149-151; git-driver.test.ts:73-91; source-inspection.ts:244-280 | resolution gated on a 40-hex match and ordered before inspect, with 12-char, 41-char and non-hex rejects |
...
| AC-0013 | met | S | git-driver.ts:55-59,153-158; storage.ts:292,401-402; git-driver.test.ts:93-106 | requested ref, resolved ref and SHA are distinct fields in distinct columns with an explicit negative on the SHA slot |
...
| AC-0028 | **not met** | W | runtime-supervisor.test.ts:696-712; source-inspection.ts:751-762 | compares environment names and never values; a second production caller runs both probes under a different environment no test reaches |
...
| AC-0154 | **not met** | S | runtime-supervisor.test.ts:853-968; source-inspection.ts:649-653; spec.md:246 | the three signalling obligations are strongly bound, but inspectInRuntime maps already-in-flight to a condition the criterion says it must not be |
...
| Criterion | Verdict | F | Bindings | Evidence and the mutation that reddens it |
| --- | --- | --- | --- | --- |
| AC-0032 | met | S | trial-result.ts:17,209-210; runtime-child.ts:201-208,1082-1100; runtime-supervisor.ts:469; source-inspection.ts:884-902; trial-result-line.test.ts:200-258 | both legs live: the child holds the canonical name, compares it against the delivered one and refuses before claiming a state root, and the Service refuses a result naming another contract. Replacing either guard with `false` reddens one case each; the child's literal is pinned against `TRIAL_CONTRACT` by a source audit, because the delivery assertion alone compares the constant with itself |
| AC-0033 | met | W | trial-result.ts:86-90; trial-result.test.ts:61-90; source-inspection.ts:686; trial-enrichment-seam.ts:68 | charset and uniqueness over 50 mints; the no-client-input clause rests on an arity check on the unwired seam, never on the live mint site |
| AC-0034 | met | S | trial-result.ts:197-199; source-inspection.ts:686-693,961-964; trial-result-line.test.ts:280-299 | `inspectInRuntime` holds the identifier it minted and passes it to the comparison; substituting `undefined` reddens the mismatch case |
| AC-0035 | met | S | trial-result.ts:187-215; source-inspection.ts:901-973; trial-result-line.test.ts:311-423 | the callerless normalizer now has its one live caller, running before any field is copied or persisted; dropping `removalOutcome` from the full-shape check reddens the non-conforming case |
| AC-0036 | met | S | trial-result.ts:201-215; source-inspection.ts:864-873,969-979; trial-result-line.test.ts:311-423 | the live refusal carries `result-invalid-studio` with a cause naming which of the six Studio-side failures fired, and the outcome carries no `result`; a case asserts all seven refusals emit pairwise-distinct diagnostics, so collapsing any two reddens it |
...
| Criterion | Verdict | F | Bindings | Evidence and the mutation that reddens it |
| --- | --- | --- | --- | --- |
| AC-0043 | met | S | inspector-locator.ts:192#locateTrustedInspector; source-inspection.ts:1044#inspectorDiagnostic; source-inspection.ts:118-124#fileDigests; storage.ts:323#readInspector; inspector-locate.test.ts:280-306#fileDigests; inspector-locate.test.ts:509-650; source-inspection-storage.test.ts:247-282#fileDigests; index.test.ts:663-673#schema_migrations | all four values are recorded, not rendered. `inspectorDiagnostic` returns the located identity beside its sentence, `settledRuntimeOutcome` carries it on the outcome, and the `source.get` projection and the `connected_sources` row both hold it — schema migration 4 adds the columns and `readInspector` parses the identity back under guard. The **two file digests** travel now, which is the clause that held this row open. Bound by a deep-equality round-trip through storage, by a digest-shape assertion at the composition, and by the built-desktop startup test that pins the applied migration list; a locator returning the pack identity with an empty digest map reddens two of the three. **"With each inspection" was held on one branch until 2026-09-25**, was widened twice that day, and the first widening's stated reason was wrong. It now holds on every path that can determine an identity: the locator runs before the first return that could skip it, so a failed materialization, a refused result line, a refused declared read, and a stopped, timed-out, cancelled or validation-refused run all record the inspector Studio held. **One termination stays `null` and is honest there** — the child refusing the request, which happens before it probes any interpreter, so `readInterpreterProbes` has nothing to read. The claim that four terminations end before the probes is false and was corrected by review round 13: the child emits its `interpreter` line before it materializes, so three of them carry the probes and were skipping the lookup rather than lacking one. Each of the three has its own case, and removing the lookup reddens all three. `connect-orient-inspector-identity-has-nowhere-to-be-recorded` is closed |
| AC-0044 | met | S | inspector-locator.ts:192-290; source-inspection.ts:953-963; inspector-locate.test.ts:434-507 | the locator is called on the live path, and a pack that does not match the pin reaches the lead as its own named mismatch rather than the generic sentence; "rather than being used" holds because nothing executes what it returns. Replacing the locator call with `false` reddens two cases |
| AC-0045 | met | S | inspector-locator.ts:215-233; source-inspection.ts:736-747,953-959; inspector-locate.test.ts:434-507 | the materialization root reaches the locator as **Studio's own computed value** — `reserveStateRoot` created the root and `per-request-state-root.ts:234` derives the child from it, so it never crosses the process boundary. **An earlier version of this cell said the opposite**, crediting the child's `started` line as the authority; nothing reads that line, and the claim inverted the trust rule this slice applies everywhere else. An inspector resolving inside the root is refused by name; dropping the root from the locator call reddens the containment case |
| AC-0046 | met | S | inspector-locator.ts:307-340,342-360; source-inspection.ts:949#readInterpreterProbes; inspector-locate.test.ts:77-222 | **the flag-versus-version defect is fixed.** `selectConformingInterpreter` decides from the version the interpreter reported, not from the `conforming` boolean the child computed, and a version that cannot be read at all is refused rather than admitted. Cases assert both directions — a conforming flag on 3.9.6 is refused, a false flag on 3.14.7 is not — so neither reading passes. Restoring the flag consumption, admitting an unreadable version, or ignoring the minor component each reddens |
| AC-0047 | met | S | source-inspection.ts:720-726; declared-read.test.ts:728; absence-proofs.test.ts:122-150 | production returns inspector-unavailable when nothing refused, and a live process-tree probe with a positive control shows no repository skill executable starts |
| AC-0048 | met | S | inspector-locator.ts:342-369; source-inspection.ts:932-948; inspector-locate.test.ts:77-222 | the requirement-naming refusal has a live caller; the production diagnostic names the required version and what each probe reported, and an absent `interpreter` line is treated as no interpreter rather than as a conforming one. Never consulting the probes reddens three cases |
...
| AC-0059 | **not met** | W | runtime-supervisor.ts:584-591; state-vocabulary.ts:206,211; source-inspection.ts:587-601 | the declaration-file branch is fully bound live, but the Studio-produced parse at :588 pushes a refused line to nonProtocolStdoutLines with no diagnostic, stop reason or routing |
...
| AC-0084 | **not met** | W | connected-source.test.ts:286-297; source-inspection.ts:656-664; source-inspection.test.ts:298-328 | the only terminates assertion is a flag on a test-supplied closure, and the pipeline cancel test injects a mock that never observes the signal |
| AC-0085 | **not met** | W | connected-source.test.ts:299-318; source-inspection-storage.test.ts:90-120; service.ts:218 | the incomplete-vs-cancelled half is bound, but no trial Runtime from a prior session is aborted: nothing in service.ts aborts in-flight runs on shutdown |
...
| AC-0104 | met | S | connected-source.test.ts:196-280; source-inspection-storage.test.ts:146-305 | the bound is measured over every provenance-marked repository-derived value, and a widened field is counted with no test edit |

### Honest states — 4 met, 10 not met

| Criterion | Verdict | F | Bindings | Evidence and the mutation that reddens it |
| --- | --- | --- | --- | --- |
| AC-0086 | **not met** | W | state-vocabulary.ts:49-55; state-projection.test.ts:36-47; source-inspection.ts:251-258 | distinct-label bound; separate-result fails because every resolution failure writes source-unavailable and source-rate-limited is unreachable |
...
| AC-0093 | met | S | state-projection.test.ts:148-178; declared-read.test.ts:969-983; source-inspection.ts:655-666 | non-tautological both directions, pins the refused-result route rather than the inspector-unavailable fall-through |
...
| AC-0096 | **not met** | W | state-vocabulary.ts:313-319; source-inspection.ts:249-258 | unreachableCondition has no production caller; the real path writes source-unavailable with no rate-limit inspection |
| AC-0097 | met | S | state-vocabulary.ts:280-283; VerdictSurface.test.tsx:200-227; source-inspection.ts:295-307 | both branches reach a rendered surface and are asserted there, value plumbed from the service |
...
| AC-0145 | **not met** | W | absence-proofs.test.ts:525-569; runtime-child.ts:855#spawn | the fetch stub wraps a fixture-only git spawn, so no Studio code runs and the real requests go through a subprocess the stub cannot see |
