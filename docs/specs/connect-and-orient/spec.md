...
| Non-originated value | any value whose bytes Studio did not produce, other than operator input the boundary charset-validates before use: a repository-derived value as AC-0039 defines it, the remote-resolved revision, any value a transport reported, and text authored by a pinned executable rather than by Studio's own code — the trusted inspector's own prose, and a child process's own diagnostic text. The item does not say *third-party*, because the second half of its gloss is the Runtime child, which is Studio's own Node process; what puts both inside the class is that Studio did not author the bytes, not who supplied the binary. The class is named once here so the rendering obligation and the sink prohibition cannot drift apart, which is how AC-0116 came to be narrower than AC-0115. **Both obligations reach a value Studio itself derived from a member of this class**, such as a commit link built from the resolved revision; stating that reach here rather than on either criterion is what keeps the two from drifting again. **Operator-supplied input is outside the class, stated here because the head would otherwise admit it and no item assigns it.** The owner, repository and ref a lead types are validated at the boundary against the *Owner / repository charset* and *Ref charset* rows before any use, and the permitted URL is built from the validated owner and repository — so reading them into this class would make AC-0116's sink prohibition forbid the operation it exists to protect. The exclusion is the boundary validation, not the typing: an operator value that reached a sink without passing those rows would have no such ground |
...
reclaim. `TZ=UTC` completes the pin rather than adding a new control.
...
| Markerless-reclaim age | 1 hour | the sweep refuses to reclaim a candidate younger than this on **either** age-gated limb of AC-0081 — a marker that cannot be parsed or does not yield both a process identity and a start time, or no marker with no entries. The maximum inspection window is 150 s — 30 s resolution plus 120 s inspection — so the threshold carries roughly twenty-four times that headroom | none — exact |
...
- Agent-Ready Studio maintainers: **accepted residual — no byte ceiling bounds materialization**, decided 2026-09-16 and recorded at `notes/verification-ledger.md#owner-decision-2026-09-16-cut-tree-bytes-bound`. The *Materialized tree bytes* bound was cut rather than restated after T5 measured the realistic writer at 208–448 MiB per 250 ms interval against a 128 MiB pass bar, with a tree at the former ceiling written inside two to three samples; the row's own rule made a higher measurement a bound failure rather than grounds to raise the bar, and the quantity is a property of the measuring host, so any restated ceiling would go stale on the next machine. **The decision was taken "for now" and is revisitable.** What remains bounding materialization is the fetch itself, `--depth 1` on history, the *Materialized file count* bound of 50,000 files, and the 120 s inspection wall-clock; none is a stated byte ceiling. The corpus keeps its `tree-bytes-bound` fixture, bound to no criterion, so a restored ceiling has its case ready. The criterion that carried the kill is retired and its number is never reused.
...
- Agent-Ready Studio maintainers: **the pinned inspector's read reach — confirmed at T6**, accepted as a residual 2026-09-16 at `notes/verification-ledger.md#owner-decision-2026-09-16-inspector-read-reach` and answered at `notes/verification-ledger.md#t6-inspector-read-reach-confirmed-2026-09-17`. The pinned inspector's source lies outside this repository and had not been read when the residual was accepted, so whether pack `core` at the pin ever opens a repository-declared path operand was not established there. It is established now, against the recorded pack name, version and file digests, before the inspector is used: it does, in three shapes, each confined by the inspector itself, and its traversal does not follow symlinks. **AC-0069's protection never rested on that traversal** — `core.symlinks=false` materializes an escaping link as a regular file holding its target string, so no symlink exists under the materialization root for any traversal to follow. This entry asserts no confinement, and it changes no criterion — AC-0054 continues to scope the permitted read surface to the Runtime's declared-value reader, routing the inspector's traversal to AC-0069 and AC-0073 for links and to the wall-clock and memory bounds.
