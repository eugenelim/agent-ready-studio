...
| Parse nesting depth | 64 levels | at the parse sites AC-0056 binds, no value is produced from a deeper document, checked before the recursion it guards | none — exact |
...
- [x] **AC-0054.** The trial reads only the permitted read surface in *Canonical values*, and a read outside it is refused. This binds the Runtime's declared-value reader; the trusted inspector's own traversal is governed by AC-0069 and AC-0073 for links and by the wall-clock and memory bounds for everything else.
- [ ] **AC-0055.** The declared-value read is bounded by the file-count and byte values in *Canonical values*, checked before reading.
- [x] **AC-0056.** Every parse of attacker-influenced structure crossing into a Studio process — the declared-value read, the inspector's output, and the northbound result line — yields no value from a document exceeding the parse nesting-depth bound, with the bound enforced before the recursion it guards. **No inspector output parse exists in this slice** — no trusted inspector runs, and the counterpart inspection contract is recorded absent in *Follow-ons* — so that limb is inapplicable here and binds the slice that first runs an inspector. The criterion is met when the declared-value read and the northbound result line hold.
- [x] **AC-0057.** No parse in AC-0056's reach — the declared-value read and the northbound result line, the inspector-output limb being inapplicable in this slice for the reason AC-0056 states — yields a value under any key in the inadmissible-parse-keys set in *Canonical values*, at any depth; every such parsed document is materialized without an inherited prototype; and normalization copies only criterion-named fields onto freshly constructed objects rather than consuming a parsed object's shape. Narrowed from "no parse" at `notes/verification-ledger.md#owner-decision-2026-09-22-ac-0056-0057-trial-boundary`, which owns the reason; the parses outside the reach are enumerated in *Follow-ons*.
...
- [ ] **AC-0059.** A parse that fails or breaches a bound yields a distinct diagnostic and no extracted value, contributes nothing partially parsed, and routes to `inspection-stopped` with the reason and attribution the *Reasons for `inspection-stopped`* table assigns. **The routing and distinct-diagnostic clauses reach the inspection parses, not the transport envelope.** `packages/protocol/src/validator.ts` parses every northbound message rather than an inspection result, and a breach there is a framing fault the transport already answers by disconnecting; the table carries no row for it and none is added, and the transport's own message is the answer, so a bound breach and a syntax failure are not distinguished there. **The no-value and no-partial-contribution clauses do bind that site**, which is the whole of what this criterion asks of it. Scoped at `notes/verification-ledger.md#owner-decision-2026-09-22-ac-0056-0057-trial-boundary`.
...
- [x] **AC-0091.** Each degraded state states whose gap it is, using the attribution the tables assign — and for `inspection-stopped`, the attribution of its specific reason rather than one answer for all of them.
- [ ] **AC-0092.** Each degraded state states whether retrying can change the answer.
- [x] **AC-0093.** No attribution crosses: a Studio-attributed state is never presented as the repository's fault, and no repository-attributed state is presented as Studio's.
...
- [x] **AC-0102.** After a Studio restart, the last verdict and its diagnostics are still readable.
...
- [x] **AC-0105.** The desktop provides a Connect repository action.
...
- **Six unguarded production parses sit outside AC-0056's and AC-0057's reach**, enumerated rather than counted: the southbound request line at `apps/studio-service/src/service.ts:1274`; Studio's ownership marker on the Service side at `apps/studio-service/src/trials/connect-and-orient-runtime/sweep.ts:127`; the `--plan` argument vector at `apps/studio-service/src/trials/connect-and-orient-runtime/runtime-child.ts:189` and the child-side ownership marker at `:415`; and Studio's persisted provenance map at `packages/storage-sqlite/src/storage.ts:297` with the stored-content helper at `:1103`. Each reconstructs structure Studio itself wrote, not repository content crossing the Runtime boundary. Each satisfies the rule recorded at `notes/verification-ledger.md#owner-decision-2026-09-22-ac-0056-0057-trial-boundary`, which owns the narrowing and states it as a rule rather than a list; this entry is the single enumeration of the sites that rule excludes. **A slice that admits repository content into any of them inherits the obligation.** One further parse outside the reach, the inspector pack's own state file at `inspector-locator.ts:134`, carries the inadmissible-key refusal and the null-prototype rebuild but **not** the parse-nesting-depth bound, and is pinned by name, version and digest under AC-0044.
- **No trusted inspector output parse exists**, so AC-0056's and AC-0057's inspector-output limb is inapplicable in this slice and binds the slice that first runs an inspector. The counterpart inspection contract's own absence is the entry gated behind RFC-0001 follow-on item 9 in this same section.
