...
alone is not enough.** The Runtime renders the marker's start time under this
allowlist; the sweep renders the live process's start time Service-side, which
AC-0023 deliberately leaves outside this scope. Pinning only the first is worse
...
and locale at its own seam, and a test renders the token both ways and compares
them, so unpinning either side reddens.
...
- Launch every subprocess with an absolute executable path and an argument
  ...
  Runtime's own Node process — whose vectors have no such configuration to carry
  and no attacker-influenced operand to place. The Node process is named here
  because the row admits it and the earlier two-item enumeration did not, which
  made the exemption read as narrower than the row it cites.
