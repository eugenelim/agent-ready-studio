...
| Materialization root | a per-request `mkdtemp` directory created inside the sweep domain at mode `0700`; the parent is fixed so the sweep can enumerate, the child is unpredictable |
...
| Progress text cadence | the `resolving` and `inspecting` text channel updates at least every 2 s and no more often than every 1 s |
| Inadmissible parse keys | `__proto__`, `constructor`, `prototype`, at any depth |
...
| Markerless-reclaim age | 1 hour | the sweep refuses to reclaim an unmarked candidate younger than this. The maximum inspection window is 150 s — 30 s resolution plus 120 s inspection — so the threshold carries roughly twenty-four times that headroom | none — exact |
...
- Confine every filesystem operation on the materialization — read, write and
  removal — to the materialization root.
...
- [ ] **AC-0024.** Every process `git` spawns carries those same names plus `GIT_CONFIG_PARAMETERS`, whose parsed key/value set equals the pinned `git` configuration; the assertion compares parsed sets, never the literal encoding.
- [ ] **AC-0025.** Every executable started anywhere in the spawned process tree, observed from the parent over the whole descendant set, is one the *Permitted executables* row admits.
...
- [ ] **AC-0054.** The trial reads only the permitted read surface in *Canonical values*, and a read outside it is refused. This binds the Runtime's declared-value reader; the trusted inspector's own traversal is governed by AC-0069 and AC-0076 for links and by the wall-clock and memory bounds for everything else.
...
- [ ] **AC-0070.** The materialization root is created per request by `mkdtemp` at mode `0700` inside the sweep domain, and no component below the fixed parent is predictable.
...
- [ ] **AC-0073.** No byte is read from a file that is not, at the moment of reading, inside the materialization root, with containment compared on a path-segment boundary so a sibling whose name extends the root is outside it.
...
- [ ] **AC-0076.** Removal refuses to descend a symbolic link and removes nothing outside the materialization root.
...
- [ ] **AC-0079.** The trial Runtime removes its materialization, its per-request home and its per-request temporary directory on success, on failure, and on receiving a termination signal.
- [ ] **AC-0080.** Each materialization root carries a Runtime-written ownership marker naming the owning process and its start time, written before any repository content enters the root, so no root holding repository content is ever unmarked. An *empty* unmarked root remains possible, because `mkdtemp` creates the directory before the marker is written; AC-0081's second limb reclaims it.
- [ ] **AC-0081.** A Runtime sweep enumerates only direct children of the sweep domain, and removes a candidate only when the entry itself — observed without following a link — is a directory owned by the current user at mode `0700`, and either it carries a marker whose recorded process and start time do not match a live process, or it carries no marker, contains no entries, and was last modified longer ago than the markerless-reclaim age in *Canonical values*. The second limb requires emptiness, so it can destroy no repository content and preserve no hostile content.
- [ ] **AC-0082.** The Studio Service invokes the sweep without itself reading under any materialization root.
- [ ] **AC-0083.** A removal failure is recorded as an explicit diagnostic rather than silently discarded.
...
- Agent-Ready Studio maintainers: **accepted residual — a Runtime can outlive an abnormal Studio Service death.** Cutting the round-3 live-orphan criterion restored a known gap in place of an unbounded one: AC-0154 holds the outer obligation in the Service, so an abnormal Service death leaves a Runtime holding a marked root that AC-0081's first limb cannot reclaim while its process is live and whose second limb requires emptiness. Bounded by the Runtime's own 150-second supervised window plus indefinite disk retention of that one materialization. Accepted 2026-09-14; the ownership decision below subsumes it.
- Agent-Ready Studio maintainers: **per-request on-disk state ownership — one open design question, blocking.** How per-request state is owned, identified and reclaimed is currently answered in five places that do not agree, and one of them is a security defect: the ownership marker AC-0080 writes lives inside the tree `git` materializes, AC-0136 protects only `.git`, and AC-0081 consumes that marker's content as its reclaim predicate — so repository content can forge ownership and either block reclaim of its own root indefinitely or trigger removal of a root still in use. The same question also governs whether the per-request `HOME` and `TMPDIR` fall inside any reclaim scope, what happens when a marker is present but unparseable, whether reclaim is one indivisible operation rather than an observe-then-delete pair, and whether anything bounds sweep-domain occupancy in aggregate. These are not five amendments; they are one decision about layout and ownership, and attempting them as separate criteria is what produced the live-orphan criterion cut in round 3, whose number is now deliberately unused. Raised by the pre-EXECUTE secure-design and adversarial reviews, rounds 2 and 3, 2026-09-14.
- Agent-Ready Studio maintainers: **slice 2**, recorded in the brief's Spec map — canonical artifact viewing, full work-state projection, capability inventory, shaping-availability explanation, refresh and staleness, projection persistence. Definition-of-done items 7, 8, 9 and 10 land there.
