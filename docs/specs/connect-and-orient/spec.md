...
The fetch-depth bound is the primary control, and it is now the only control
bounding how much a repository can bring: bounding the input is why the file-count
tolerance can be small enough to state honestly, and why cutting the byte ceiling
leaves a bounded gap rather than an open one.
