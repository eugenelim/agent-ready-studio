...
writes the marker, and AC-0082's split has the sweep performed in the child rather
than the Service. The Service-side seam is `readProcessStartTime`, which the
...
table above. A test forces the ambient zone to one that is never the pin, derives
the Runtime's side from the allowlist builder and reads the Service's side
through its own seam, so unpinning **either** side reddens **whatever the host's
own zone is** — including UTC, where an earlier version of that case was green
because it compared the reader against a rendering it pinned itself.
...
- **Disposal, cancellation and sweep (AC-0077, AC-0078, AC-0079, AC-0080, AC-0081, AC-0082, AC-0083, AC-0084, AC-0085, AC-0159)** — TDD, exercised by an integration test. Group-signalled lifecycle transitions and the marker-based sweep predicate. **AC-0159 is here rather than with the environment group** because what it constrains is the byte equality AC-0081's first limb depends on, not the trial tree's environment: its case forces the ambient zone away from the pin, derives the Runtime's rendering from the allowlist builder and reads the Service's through its own seam, so it fails when either side is unpinned, on any host zone.
