...
rendering, which catches a call site that stops using the pinned set — on any
host, because both compared values are then explicit and neither falls back to
the host's own zone.
